...
import coordinate_transformation.Gauss_projection as gapro
from coordinate_transformation.gps_view import draw_gps
...
def photo_parameter(jpg_data_dir):
    ...
    photo_yaw = (float(photo_yaw[1:len(photo_yaw) - 1])-0)/-180*np.pi
    photo_pitch = (float(photo_pitch[1:len(photo_pitch) - 1])+90.0)/-180*np.pi
    # photo_roll = 0
    # photo_yaw = 0
    # photo_pitch = 0
    print('roll',photo_roll,'yaw', photo_yaw, 'pitch',photo_pitch)
    # print('sin(pi)',np.sin(180))
    #相机内参赋值
...
def get_rice_objection(jpg_data_dir, txt_data_dir):
    ...
    x_center_list = []
    y_center_list = []
    scale_world_all = [[4192, 1550], [618, 1729]]
    for i , j in enumerate(scale_world_all):
        x_center_list.append(j[0])
        y_center_list.append(j[1])
    # 质心显示在图片上
    img = cv2.imread(jpg_data_dir)
    for i in range(0,len(x_center_list)):
